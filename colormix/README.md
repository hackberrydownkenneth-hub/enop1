...
- **ロゴ画像 (`logo.png`) は加工していません。** もらった原本から余白を取り除いた
  だけで、絵柄のピクセルは1つも変えていません。背景も透過のままです。
- ヘッダーに原寸比のまま表示しています（168px 幅）。台紙や枠は付けていません。
- **フッターは文字での表記**にしています。ダークモードでは背景が暗く、黒い
  ロゴが埋もれてしまうためです。**ロゴの色を反転させたり、暗い背景に無理に
  置いたりはしません。**
- 読み上げ用に `aria-label="DRIVE BLUE HONG KONG"` を付けています。
- 1ファイル版では、`build.js` が CSS 内の PNG を data URI に埋め込みます。

原本は 327×60 相当です。ヘッダーで 168px 幅（約2倍の密度）で表示しているので
通常は問題ありませんが、**もっと大きく使うなら SVG か高解像度の PNG**に
差し替えてください。`logo.png` を置き換えるだけで反映されます。
白抜き版があれば、ダークモードのフッターにもロゴを出せます。
