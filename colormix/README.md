...
画面上部のボタンで **日本語 ⇄ 繁體中文** をいつでも切り替えられます。
初回は端末の言語設定から自動で選ばれ（`zh-*` / `yue-*` なら繁體中文）、以降は選んだ言語を記憶します。

中文表示は**書面語**（口語の「嘅」「咗」などは使わない）で統一し、用語は香港のサロンで使うものに合わせています。
...
- **レシピ保存** — よく使う配合に名前をつけて保存。**利用者の端末内（localStorage）
  にのみ保存**され、運営者にも第三者にも送信されません。その旨を画面にも明記して
  います（`recipes.note`）。同じ名前で保存すると上書きの確認が出ます（最大30件）
...
登録すると端末に記録され、**2回目からは出ません**。日本語・繁體中文の両方で表示されます。
...
| ファイル | 用途 |
|----------|------|
| `logo.png` | ブランドカラー版 (#2E3092)。明るい背景用 |
| `logo-white.png` | 白版。紫のヘッダーとダークモード用 |

- ロゴはベクター PDF から書き出しています（700×135、64色パレットPNG）。
  文字のアウトラインもそのままなので、表示サイズでは完全に鮮明です。
- **`HONG KONG` はこちらで組んでいます。** PDF の原本には入っていないため、
  以前もらったロゴ画像から比率を実測して同じ配置で再現しました。

  | 項目 | 実測した比率 |
  |------|------------|
  | 文字高さ | DRIVE BLUE の 0.630 倍 |
  | 上の間隔 | DRIVE BLUE の 0.222 倍 |
  | 文字幅 | DRIVE BLUE の 0.573 倍（字間で調整） |
  | 位置 | DRIVE BLUE の中心に揃える |

  書体は Liberation Serif（Times 互換）です。正式な書体の指定があれば
  教えてください。組み直します。
- 白版はブランドカラーの部分を白にし、エンブレム内側の白は透明にしています
  （背景が透ける正しい白抜き）。
- ヘッダーは常に白版、フッターは `--logo` トークンで背景に応じて切り替えます。

再生成する場合は、PDF から 1100px 幅で描画 → HONG KONG を合成 → 700px に縮小 →
64色に減色、という手順です。SVG も書き出せますが 329KB（gzip 118KB）と重く、
ページ全体の5倍近くになるため採用していません。

### リンクを共有したときのプレビュー

WhatsApp や Instagram にURLを貼ったときに出るタイトル・説明・画像は、
`index.html` の `<title>` と OGP タグから読まれます。**共有元は JavaScript を
実行しないので、言語切り替えは効きません。** そのため主な利用者に合わせて
**繁體中文で固定**しています（アプリを開いた後のタブ名は、従来どおり言語に
追従します）。

共有画像は `ogp.png`（1200×630）です。ブランドカラーの紺 `#2E3092` を地に、
エンブレム（`emblem.png`）を透かしとして置き、実際の計算例を1つ載せています。
文字は Noto Sans TC。作り直す場合は同じ寸法で差し替えてください。

`emblem.png` はエンブレム単体のベクター原本から書き出したものです（686×686）。
共有画像の素材として置いてあり、ページからは読み込んでいません。
...
|------|------|
| ヘッダーのロゴ | タップで公式サイトへ |
| フッター | 「公式サイト」「Instagram」「WhatsApp」のボタン |
| 登録完了画面 | WhatsApp で連絡先を渡してもらう導線（下記） |
| 登録画面 | `register.contact` に入れた問い合わせ先 |

#### WhatsApp は「相手から送ってもらう」

電話番号をこちらから聞くと、無料ツールの登録としては警戒されます。かわりに
**登録完了直後に `wa.me` のリンク**を出し、相手からメッセージを送ってもらいます。

- こちらから電話番号を要求しない
- 送ってくれた時点で、その人の番号が WhatsApp に残る
- 本文には登録した Instagram ID を埋め込んであるので、回答データと紐付けられる
- 相手から来た会話なので、こちらが返信するのが自然

`links.whatsapp` に**国番号付き・記号なし**の番号を入れると有効になります
...
> 保存していないと届きません。個別のやり取り用と考えてください。
...
の 2 手順で済みます。キーの過不足や翻訳し忘れは `i18n.test.js` が検出します。
